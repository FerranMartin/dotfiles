...
# Documentation:
# @raycast.description Increase video playback speed by 0.25x
...
set jsCode to "var v = document.querySelector('video'); if(v){ var prev = v.playbackRate; v.playbackRate = Math.min(v.playbackRate + 0.25, 4); 'Changed from ' + prev + ' to ' + v.playbackRate; } else { 'No video element found'; }"
